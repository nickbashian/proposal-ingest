...
## MVP-05b — AI-first ingestion and exception review

**Outcome:** A newly captured and extracted proposal runs through AI-assisted classification without per-document hand entry. Source-backed tags and safe policy decisions produce a curation plan; only genuine exceptions reach a person. Depends on merged 05 and precedes publication in 06.

**Components:** Durable classification jobs after successful extraction; bounded Bedrock task calls and deterministic rule/prior-decision reuse; source-grounded fact and treatment recommendations; explicit policy for automatic resolution versus escalation; review workload accounting; mock end-to-end ingest path. Reuse the MVP-05 decision ledger, model adapter, reservations, and plan builder rather than maintaining a second metadata authority. The older CLI's Bedrock analysis is reference behavior, not a parallel production database.

**Acceptance:**

- **05b-A:** A completed source capture and extraction schedules/resumes classification for active source versions and proposal memberships. A synthetic local run goes from source file through extraction, AI-tagged facts, automatic or queued decisions, and a current curation plan using the real application services. Reruns, worker crashes, changed source bytes, changed model/prompt/policy revisions, cancellation, and stale completion do not duplicate decisions, lose corrections, or activate an obsolete plan.
- **05b-B:** Bedrock classification emits bounded, typed suggestions for source role, authorship, version/status, content use, claim type, chemistry/conditions, temporal meaning, treatment, and sensitivity where the source supports them. Persist source version, exact evidence unit IDs, rationale, model/prompt/schema revisions, usage, and an explicit unknown/abstain state. Names, tracker administrative fields, and model confidence alone cannot establish scientific facts or publication permission. Malformed, missing-confidence, or provider-error results remain visible and unresolved.
- **05b-C:** Define a versioned automatic-resolution policy. Automatically resolve routine, source-supported, non-sensitive decisions only when the policy permits them; carry forward applicable human decisions and explicit collection prohibitions. Escalate weak or conflicting evidence, uncertain identity/status, sensitive material, destructive exclusions, evidence-critical scientific claims, and proposed voice approval. A model prediction never directly writes a publication artifact. Every automatic decision has evidence, rationale, scope, and resolver revision; human correction overrides it and invalidates affected work.
- **05b-D:** The review queue is generated from exceptions, not one approval request per document or passage. Show a proposal-level summary of automatic tags, included/excluded/pending counts, reasons, and estimated review effort, with drill-down to original sources. Keep every unresolved critical exception discoverable and scoped; neither display caps nor model omission imply clearance. Record the number of documents/units processed automatically and the number/time of human decisions required.
- **05b-E:** Synthetic tests include routine technical evidence that clears without a click, ambiguous final/draft status, contradictory measurements, a sensitive passage, a partial inclusion, an unsupported numeric claim, a voice sample, a failed model call, and a changed source after approval. Inspect planned and locally published bytes and evidence packets: only eligible material appears, and pending/excluded material remains absent. The path must work with `--mock-bedrock` or equivalent blocked-provider fixtures and no private data.
- **05b-F:** Keep live provider calls opt-in with a per-run cap and persistent reservations. Real model choice, confidence bands, and automatic-clearance thresholds remain pending private calibration in 09; an uncalibrated model cannot silently clear live private material. The live acceptance target is at least 80% of supported, non-sensitive passages resolved without human action and at most ten substantive questions across the three seed families except genuine critical exceptions, while all retention, exclusion, and scientific-fidelity gates in `ACCEPTANCE.md` still pass. If the target fails, report the workload and failure causes rather than relaxing safety gates or claiming AI-first operation.

**Demo/evidence:** Show one synthetic proposal folder ingested through automatic tagging, an inspectable exception queue, a human correction, and a revised plan; report automatic coverage and review counts. **Owner input:** None for offline implementation; Nicholas validates real scientific/voice judgments and review burden during 09. **Rollback:** Disable new classification dispatch, keep prior decisions and source versions, and invalidate affected plans; never replay an old automatic decision over a newer human correction. **Boundary:** No live tenant or paid call is needed for this PR. MVP-06 owns physical S3/KB publication.

## MVP-06 — Curated publication and Managed KB retrieval adapter

**Outcome:** Search receives only eligible material from the AI-first ingestion and review path, and indexing state is explicit. Depends on 05b. Suggested lead: Astra.
...
- **06-H:** Exercise the MVP-05b source-to-classification-to-plan path as the input to publication. An automatically cleared passage may publish only under the current calibrated policy and current plan; unresolved exceptions and stale source/model/policy generations never reach S3, the KB, or drafting context. Show the reviewer correction invalidating an already published eligible passage.
...
- **08-D (live):** Ingest one seed family through source sync, extraction, AI tagging, exception review, approved publication, completed KB ingestion, normalized retrieval/citations, and draft/edit/export in the deployed UI. Repeat sync without duplicate work. Update then retire a disposable test artifact and prove stale results cannot enter a new model packet. Keep automatic clearance of live private passages disabled until the calibrated policy is approved in 09.
...
- **09-C:** Complete scoped decision persistence/reopening tests with real reruns. Measure source-backed automatic tag coverage, the share of supported non-sensitive passages resolved without human action, uncapped unresolved counts, substantive questions, corrections, and time spent reviewing. Demonstrate at least 80% review-free resolution and at most ten substantive questions across the three seed families except genuine critical exceptions while meeting all scientific and exclusion gates. Investigate a miss; do not hide questions, weaken gates, or claim AI-first ingestion when the workload target fails.
- **09-D:** Run the three-way classifier comparison on source-checked labels, record actual models/prompt versions/cost, select routes and thresholds, and document abstentions. Calibrate and then explicitly enable the versioned automatic-clearance policy only for demonstrated safe cases; rerun frozen inclusion/exclusion tests before claiming the review-free rate. If Jev remains unavailable or its terms unresolved, ship the working Bedrock path with an explicitly incomplete Jev experiment; never claim a three-way result.
