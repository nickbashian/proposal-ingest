...
The extraction fingerprint uses source SHA-256, parser selection and installed parser package version, extractor/schema revision, and relevant limits. Re-extraction creates new run/unit IDs; old units and saved citation URLs stay intact. The audit command samples source versions across families using IDs and counts without exposing source text. No original source tree writes or cloud calls occur.
