...
Apply migration `0017_classificationresult` after a database backup. It adds operational model results and measured browser review time. Plans created before this migration have no classification configuration fingerprint and are treated as stale. Rebuild them against the current source version before publication or drafting; do not copy a fingerprint onto an unverified old plan. A new source version withdraws eligible artifacts from the prior version, including artifacts from the older local workflow. Reauthorize and publish the current version after review. Keep prior `DecisionEvent` rows and source snapshots during rollback; do not reverse the migration over retained data or reactivate withdrawn artifacts without current eligibility checks.
